# Bloomstead

**Automate the rebirth of a world.**

![Bloomstead, a blooming meadow with a small sap factory](https://raw.githubusercontent.com/Bloodstead/.github/main/profile/media/header_capsule.png)

Bloomstead is a cozy 2D automation game where your factory doesn't pollute, it *heals*.
Harvest sap from crystals, chain extractors, belts and bloomers, and watch a withered
world turn green again, tile by tile, as your little production lines hum along.

Made with [Godot 4](https://godotengine.org) by a solo developer in France. Every sprite,
sound and song in the game is generated procedurally, without imported assets.

## Trailer

![A wave of sprinklers reblooming the whole level](https://raw.githubusercontent.com/Bloodstead/.github/main/profile/media/teaser_wave.gif)

**[Watch the full trailer with sound](https://bloodstead.github.io/bloomstead-privacy/)**

## What's inside

- Nine machines that chain into small production lines: extractors, belts, splitters, tunnels, sprinklers, pumps.
- A handmade campaign, then endless generated islands, plus a daily challenge, across nine biomes.
- You can also plant by hand. Carry a seed anywhere; the machines do the scale, your hands do the details.
- Permanent upgrades, achievements, and a free garden mode with decorations and a photo mode.
- English and French, remappable keys, a reduced-motion option, full touch support.

## Screenshots

[![A small factory reblooming a meadow](https://raw.githubusercontent.com/Bloodstead/.github/main/profile/media/screenshot_01_factory_meadow.jpg)](https://bloodstead.github.io/bloomstead-privacy/)
[![A conveyor chain crossing the desert](https://raw.githubusercontent.com/Bloodstead/.github/main/profile/media/screenshot_03_desert.jpg)](https://bloodstead.github.io/bloomstead-privacy/)
[![The world map and its golden path](https://raw.githubusercontent.com/Bloodstead/.github/main/profile/media/screenshot_05_world_map.jpg)](https://bloodstead.github.io/bloomstead-privacy/)

More on the [game page](https://bloodstead.github.io/bloomstead-privacy/).

## Where to play

The game is in the final stretch before release. Planned platforms:

| Platform | Status |
|---|---|
| PC (Steam, itch.io) | In preparation, Steam page and wishlist coming soon |
| Android (Google Play) | In preparation |
| Browser (web portals) | In preparation |

## Links

- [Game page, trailer and screenshots](https://bloodstead.github.io/bloomstead-privacy/)
- [Release notes / Notes de version](https://bloodstead.github.io/bloomstead-privacy/versions.html)
- [Privacy policy / Politique de confidentialité](https://bloodstead.github.io/bloomstead-privacy/privacy.html)
- Contact: valentin.damlencourt@gmail.com

---

*The game's source repository is private. Releases and news are published here.*
